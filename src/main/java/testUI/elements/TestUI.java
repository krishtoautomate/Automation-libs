...
public class TestUI {
  ...
  public static UIElement E(String accesibilityId) {
    if (accesibilityId.contains(": ")) {
      return new Element(null, null, null, 0, false, accesibilityId, accesibilityId);
    }
    return new Element(null, null, null, 0, false, "accessibilityId: " + accesibilityId,
  ...
  protected String getStringElement(String accesibilityIdiOS, String accesibilityId, By iOSElement,
      By AndroidElement, By SelenideElement) {
  ...
  protected WebElement getMobileElement(String locator) {
    switch (locator.split(": ")[0]) {
      case "accessibilityId":
        return driver.findElementByAccessibilityId(locator.split(": ")[1]);
      case "className":
        return driver.findElementByClassName(locator.split(": ")[1]);
      // case "androidUIAutomator":
      ...
      case "name":
        return driver.findElementByName(locator.split(": ")[1]);
      case "xpath":
        return driver.findElementByXPath(locator.split(": ")[1]);
      case "id":
        return driver.findElementById(locator.split(": ")[1]);
      case "css":
        return driver.findElementByCssSelector(locator.split(": ")[1]);
      default:
        // UIAssert("The type of locator is not valid! " + locator.split(": ")[0], false);
        return driver.findElementByName("");
    }
  }

  protected MobileElement getElement(String accesibilityIdiOS, String accesibilityId, By iOSElement,
      By element, int index, boolean collection) {
    try {
      if (collection) {
        if (!getLocator(accesibilityIdiOS, accesibilityId).isEmpty()) {
        ...
        return (MobileElement) driver.findElements(getAppiumElement(iOSElement, element))
            .get(index);
      }
      if (!getLocator(accesibilityIdiOS, accesibilityId).isEmpty()) {
      ...
      return (MobileElement) driver.findElement(getAppiumElement(iOSElement, element));
    } catch (Throwable e) {
  ...
  protected MobileElement getElementWithoutException(String accesibilityIdiOS,
      String accesibilityId, By iOSElement, By element, int index, boolean collection) {
    if (collection) {
      if (!getLocator(accesibilityIdiOS, accesibilityId).isEmpty()) {
        return (MobileElement) getMobileElementList(
            getAccesibilityId(accesibilityIdiOS, accesibilityId)).get(index);
      }
      return (MobileElement) driver.findElements(getAppiumElement(iOSElement, element)).get(index);
    }
    if (!getLocator(accesibilityIdiOS, accesibilityId).isEmpty()) {
