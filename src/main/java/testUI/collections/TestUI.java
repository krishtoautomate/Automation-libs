...
public class TestUI implements UICollection {
  ...
  public static UICollection EE(By element) {
    return new TestUI(element, element, element, null, 0, "", "");
  }

  public static UICollection EE(UIElement... element) {
    return new TestUI(null, null, null, element, 0, "", "");
  }

  public static UICollection EE(String accessibilityId) {
    if (accessibilityId.contains(": ")) {
      return new TestUI(null, null, null, null, 0, accessibilityId, accessibilityId);
    }
    return new TestUI(null, null, null, null, 0, "accessibilityId: " + accessibilityId,
  ...
  public static UICollection EEx(String xpath) {
    return new TestUI(By.xpath(xpath), By.xpath(xpath), By.xpath(xpath), null, 0, "", "");
  }

  protected TestUI(By element, By SelenideElement, By iOSElement, UIElement[] elementUI, int index,
      String accessibilityId, String accessibilityIdaOS) {
  ...
  public UICollection setSelenideCollection(By SelenideElement) {
    return new TestUI(element, SelenideElement, element, elementUI, index, accesibilityId,
        accesibilityIdiOS);
  }

  @Override
  public UICollection setIOSCollection(By iOSElement) {
    return new TestUI(element, SelenideElement, iOSElement, elementUI, index, accesibilityId,
        accesibilityIdiOS);
  }
  ...
  public UICollection setIOSCollection(String accessibilityIdiOS) {
    if (accessibilityIdiOS.contains(": ")) {
      return new TestUI(element, SelenideElement, null, elementUI, index, accesibilityId,
          accessibilityIdiOS);
    }
    return new TestUI(element, SelenideElement, null, elementUI, index, accesibilityId,
  ...
  public UICollection setAndroidCollection(By element) {
    return new TestUI(element, SelenideElement, iOSElement, elementUI, index, "",
        accesibilityIdiOS);
  }
  ...
  public UICollection setAndroidCollection(String accessibilityId) {
    if (accessibilityId.contains(": ")) {
      return new TestUI(null, SelenideElement, iOSElement, elementUI, index, accessibilityId,
          accesibilityIdiOS);
    }
    return new TestUI(null, SelenideElement, iOSElement, elementUI, index,
        "accessibilityId: " + accessibilityId, accesibilityIdiOS);
  }
    ...
    if (!getAccessibilityId().isEmpty() && !getAccessibilityId().split(": ")[0].isEmpty()) {
      switch (getAccessibilityId().split(": ")[0]) {
        case "accessibilityId":
          return driver.findElementsByAccessibilityId(getAccessibilityId().split(": ")[1]);
        case "className":
          return driver.findElementsByClassName(getAccessibilityId().split(": ")[1]);
        ...
        case "name":
          return driver.findElementsByName(getAccessibilityId().split(": ")[1]);
        case "xpath":
          return driver.findElementsByXPath(getAccessibilityId().split(": ")[1]);
        case "id":
          return driver.findElementsById(getAccessibilityId().split(": ")[1]);
        case "css":
          return driver.findElementsByCssSelector(getAccessibilityId().split(": ")[1]);
        default:
          // UIAssert("The type of locator is not valid! " + getAccessibilityId().split(": ")[0],
          // false);
          return new ArrayList();
      }
    }
    return driver.findElements(getAppiumElement());
  }
  ...
  public UIElement get(int i) {
    if (elementUI != null)
      return elementUI[i];
    return new Element(element, SelenideElement, iOSElement, i, true, accesibilityId,
        accesibilityIdiOS);
  }
  ...
  public UIElement first() {
    if (elementUI != null)
      return elementUI[0];
    return new Element(element, SelenideElement, iOSElement, 0, true, accesibilityId,
        accesibilityIdiOS);
  }
  ...
  // takeScreenshotsAllure();throw new TestUIException("No visible element with this selector:
  // "+element.toString());
  ...
  public UICollection waitUntilAllVisible(int seconds) {
    ...
    // putLogDebug("Collection of elements visible after %d ± %d ms", lastCommandTime,
    // timeErrorBar);
  ...
  public UIElement findByText(String text) {
    ...
    takeScreenshotsAllure();
    throw new TestUIException("No visible element with that text '" + text + "' and this selector: "
        + element.toString());
  ...
  public UIElement findByValue(String value) {
    ...
    while (System.currentTimeMillis() < end) {
      if (elementUI != null) {
        ...
        for (int i = 0; i < size(); i++) {
          // if (containsAttribute(getAppiumElement(), getAccessibilityId(), i, "value", value)) {
          return new Element(element, SelenideElement, iOSElement, i, true, accesibilityId,
              accesibilityIdiOS);
        }
      }
    ...
    // while (System.currentTimeMillis() < end) {
    // for (int i = 0; i < size(); i++) {
    // if ($$(SelenideElement).get(i).is(com.codeborne.selenide.Condition.value(value))) {
    // return new Element(element, SelenideElement, iOSElement, i, true, accesibilityId,
    // accesibilityIdiOS);
    // }
    // }
    ...
    // }
    // takeScreenshotsAllure();
    throw new TestUIException("No visible element with that value '" + value
        + "' and this selector: " + element.toString());
  }
