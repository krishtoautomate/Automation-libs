...
public class GridTestUI {
  ...
  public void setAppiumAndroidConfiguration() {
    ...
    try {
      ...
      throw new TestUIException(
          "Could not retrieve remote configuration with testUI " + "server: \n" + e.toString());
    } finally {
      close();
    }
  }
  ...
  private void attachShutDownHookSReleaseDevice(String url, String UDID) {
    Runtime.getRuntime().addShutdownHook(new Thread(() -> setAppiumRelease(url, UDID)));
  }
  ...
  private void setIOSConfiguration(JSONObject json) {
    try {
      ...
      throw new TestUIException(
          "there is no device available with this specifications: \n" + setJsonBody());
    }
  }
  ...
  private void setAndroidConfiguration(JSONObject json) {
    try {
      ...
      throw new TestUIException(
          "There is no node available for the specified parameters " + "\n" + setJsonBody());
    }
  }
  ...
  private void setDesktopConfiguration(JSONObject json) {
    try {
      ...
      // Configuration.selenideBrowserCapabilities.setCapability(CapabilityType.PLATFORM_NAME,
      // getPlatform(json));
      // }
