...
public class AndroidDeviceFinder implements DeviceFinder<Android> {
  ...
  private final String ADB_LIST_SHELL_COMMAND = Constants.ADB + " devices";
  private final String ADB_SERIAL_NUMBER_PUT_SHELL_COMMAND =
      Constants.ADB + " -s serialNumber shell getprop";
  private final String ADB_INTEGRATED_CIRCUIT_CARD_IDENTITY_SHELL_COMMAND = Constants.ADB
      + " -s serialNumber shell dumpsys CustomFrequencyManagerService | grep 'mIccid sim'";
  ...
  public Map<String, Object> readDeviceInfo(String localPath) throws IOException {
    ...
    while ((line = reader.readLine()) != null) {
      ...
      Process deviceDetailInfoProcess = ProcessHelper
          .runTimeExec(String.format("%s%s", localPath, ADB_SERIAL_NUMBER_PUT_SHELL_COMMAND)
      ...
      while ((infoLine = infoReader.readLine()) != null) {
        try {
          ...
          if (infoLine.contains(":")) {
            ...
            parentMap.put(detailInfo[0].replace(".", "").replace("_", "").replace("-", ""),
                detailInfo[1]);
          }
        } catch (Exception e) {
        }
