		...
		if ("Android".equalsIgnoreCase(platForm)) {
			...
			while (retry > 0) {
				try {
					...
					desiredCapabilities = capabilitiesManager.loadJSONCapabilities(Constants.ANDROID_CAPABILITIES,
							"ANDROID");
					...
					tlDriver.set(
							new AndroidDriver<MobileElement>(new URL(server.getUrl().toString()), desiredCapabilities));
					break;
				} catch (Exception e) {
