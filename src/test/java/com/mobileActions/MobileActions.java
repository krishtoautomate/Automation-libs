...
public class MobileActions {
	...
	/**
	 * type each char of String in Android text-box
	...
	public void scrollDowntoFind(By Locator) {
		...
		for (int i = 0; i < 5; i++) {
			try {
				...
				touchAction().longPress(ElementOption.point(x, y_start)).moveTo(ElementOption.point(x, y_end)).release()
						.perform();
			}
		}
	}
	...
	public void scrollbyCoords(int x_start, int x_end, int y_start, int y_end, int x_scrolls) {
		for (int i = 0; i < x_scrolls; i++) {
			try {
				...
				touchAction().longPress(ElementOption.point(x_start, y_start)).moveTo(ElementOption.point(x_end, y_end))
						.release().perform();
	...
	public void toggleWifiOFF(String platForm) {
		if ("ios".equalsIgnoreCase(platForm)) {
			try {
				...
				((AppiumDriver<MobileElement>) driver).resetApp();
//				   .activateApp(
//						   ((AppiumDriver<MobileElement>) driver).getCapabilities().getCapability("bundleId").toString());
	...
	public void toggleWifiON(String platForm) {
		if ("ios".equalsIgnoreCase(platForm)) {
			...
			try {
				// Restart app
				((AppiumDriver<MobileElement>) driver).activateApp(
						((AppiumDriver<MobileElement>) driver).getCapabilities().getCapability("bundleId").toString());
				log.info("App Restarted");
			} catch (Exception e) {
				// ignore
