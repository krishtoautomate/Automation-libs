...
public class TestFlightApp extends BaseObjs<TestFlightApp> {
	...
	public WebElement get_apps_h1() {
		WebElement ele = null;
		try {
			wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(apps_h1));
			ele = driver.findElement(apps_h1);
		} catch (Exception e) {
			logmessage(Status.FAIL, "'Apps' header text -  Not Found in 'TestFlight' app");
			Assert.fail("'Apps' header text -  Not Found in 'TestFlight' app");
		}
		return ele;
	}
